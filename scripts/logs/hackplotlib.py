...
class HackPlot(object):
    def __init__(self, parent, move_mode, show_path, use_event_bars, log_levels):
        self.canvas = Canvas(parent, bg="white")
        self.listbox = Listbox(parent, width=20, bg="white", state="disabled", disabledforeground="black")
    ...
    def draw_boundary(self):
        for x in self.get_ticks(self.x_low, self.x_high):
            ...
            self.canvas.create_line(screen_x, self.canvas.winfo_height() * 0.1, screen_x, self.canvas.winfo_height() * 0.9, fill="light grey")
            self.canvas.create_text(screen_x, self.canvas.winfo_height() * 0.95, text=str(x))
        for y in self.get_ticks(self.y_low, self.y_high):
            _, screen_y = self.screen_coords(0, y)
            self.canvas.create_line(self.canvas.winfo_width() * 0.1, screen_y, self.canvas.winfo_width() * 0.9, screen_y, fill="light grey")
            self.canvas.create_text(self.canvas.winfo_width() * 0.05, screen_y, text=str(y))
        self.canvas.create_line(self.canvas.winfo_width() * 0.1, self.canvas.winfo_height() * 0.1,
                         self.canvas.winfo_width() * 0.9, self.canvas.winfo_height() * 0.1,
                         self.canvas.winfo_width() * 0.9, self.canvas.winfo_height() * 0.9,
                         self.canvas.winfo_width() * 0.1, self.canvas.winfo_height() * 0.9,
                         self.canvas.winfo_width() * 0.1, self.canvas.winfo_height() * 0.1)
    ...
    def draw_legend(self):
        ...
        legend_height += 10
        if self.plots != {}:
            self.legend.append(self.canvas.create_rectangle(self.canvas.winfo_width() - legend_width, 0, self.canvas.winfo_width(), legend_height, fill="white"))
        index = 0
        for name in self.plots:
            self.legend.append(self.canvas.create_text(self.canvas.winfo_width() - 5, index * 15 + 5, text=name, fill=self.plots[name].color, anchor=NE))
            index += 1
    ...
    def get_ticks(self, bound_low, bound_high):
        ...
        if size / step > 6:
            ...
            step /= 2
        return [i * step for i in range(int(math.ceil(bound_low / step)), int(math.ceil(bound_high / step + 1e-3)))]

    def screen_coords(self, x, y):
        screen_x = ((x - self.x_low) / self.plot_width * 0.8 + 0.1) * self.canvas.winfo_width()
        screen_y = ((self.y_high - y) / self.plot_height * 0.8 + 0.1) * self.canvas.winfo_height()
        return (screen_x, screen_y)

    def plot_coords(self, x, y):
        plot_x = (float(x) / self.canvas.winfo_width() - 0.1) / 0.8 * self.plot_width + self.x_low
        plot_y = self.y_high - (float(y) / self.canvas.winfo_height() - 0.1) / 0.8 * self.plot_height
    ...
    def add_textlog(self, strings, name):
        ...
        self.logs[name] = [(int(re.search("\\d+", string).group()), string[:-1]) for string in strings]
        self.show_textlogs()
    ...
    def draw_plot(self, name):
        ...
        points = []
        for x, y in itertools.izip(self.plots[name].xvals, self.plots[name].yvals):
            if x >= self.x_low and x <= self.x_high and y >= self.y_low and y <= self.y_high:
                screen_x, screen_y = self.screen_coords(x, y)
                points.append(self.canvas.create_rectangle(screen_x - 1, screen_y - 1, screen_x + 1, screen_y + 1, outline=color))
        ...
        for timestamp, string in logs:
            ...
            if timestamp >= self.x_low and timestamp <= self.x_high and level_included:
                ...
                if self.use_event_bars.get():
                    self.event_bars.append(self.canvas.create_line(screen_x, self.canvas.winfo_height() * 0.1,
                                                                   screen_x, self.canvas.winfo_height() * 0.9, fill="grey"))
    ...
    def mouseup(self, event):
        ...
        if self.move_mode.get() == "zoomin" and self.mouse_start != None:
            ...
            self.x_low -= (selection_x_low - self.x_low) * self.plot_width / (selection_x_high - selection_x_low)
            self.x_high += (self.x_high - selection_x_high) * self.plot_width / (selection_x_high - selection_x_low)
    ...
    def mousemove(self, event):
        ...
        if self.move_mode.get() == "pan" and self.mouse_start != None:
            ...
            self.redraw(None)
        elif self.move_mode.get() in ["zoomin", "zoomout"] and self.mouse_start != None:
            self.canvas.delete(self.zoom_trace)
            mouse_start_screen = self.screen_coords(self.mouse_start[0], self.mouse_start[1])
            ...
            self.zoom_trace = self.canvas.create_rectangle(bound_left, bound_top, bound_right, bound_bottom)
        if self.coord_text != None:
            self.canvas.delete(self.coord_text)
        self.coord_text = self.canvas.create_text(10, 10, text="({:.6}, {:.6})".format(mouse_end[0], mouse_end[1]), anchor=NW)

...
StateSpaceController<kNumInputs, kNumStates,
    ...
    : K_{Eigen::Matrix<double, kNumInputs, kNumStates>::Zero()},
      ...
      u_min_{Eigen::Matrix<double, kNumInputs, 1>::Constant(
          -std::numeric_limits<double>::infinity())},
      u_max_{Eigen::Matrix<double, kNumInputs, 1>::Constant(
          std::numeric_limits<double>::infinity())} {}
...
StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::Update(
  ...
  Eigen::Matrix<double, kNumInputs, 1> u =
      K() * (r_ - x) + Kff() * (r_ - A() * r_);
...
StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::r() const {
  return r_;
}

template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
Eigen::Matrix<double, kNumStates, 1>&
StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::r() {
  return r_;
}

template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
double StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::r(
    uint32_t i) const {
  return r_(i);
}
...
template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
const Eigen::Matrix<double, kNumInputs, 1>&
StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::u_min() const {
  return u_min_;
}

template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
Eigen::Matrix<double, kNumInputs, 1>&
StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::u_min() {
  return u_min_;
}

template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
double StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::u_min(
    uint32_t i) const {
  return u_min_(i);
}
...
template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
const Eigen::Matrix<double, kNumInputs, 1>&
StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::u_max() const {
  return u_max_;
}

template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
Eigen::Matrix<double, kNumInputs, 1>&
StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::u_max() {
  return u_max_;
}

template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
double StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::u_max(
    uint32_t i) const {
  return u_max_(i);
}
...
template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
double StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::K(
...
template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
double StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::Kff(
    uint32_t i, uint32_t j) const {
  return Kff_(i, j);
}
...
template <uint32_t kNumInputs, uint32_t kNumStates, uint32_t kNumOutputs>
double StateSpaceController<kNumInputs, kNumStates, kNumOutputs>::A(
    uint32_t i, uint32_t j) const {
  return A_(i, j);
}

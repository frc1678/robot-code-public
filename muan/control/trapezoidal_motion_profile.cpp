...
TrapezoidalMotionProfile::TrapezoidalMotionProfile(MotionProfileConstraints constraints,
                                                   MotionProfilePosition goal, MotionProfilePosition initial)
...
MotionProfilePosition TrapezoidalMotionProfile::Calculate(muan::units::Time t) const {
  ...
  if (t < end_accel_) {
    ...
    result.position += (initial_.velocity + t * constraints_.max_acceleration / 2.0) * t;
  } else if (t < end_full_speed_) {
    result.velocity = constraints_.max_velocity;
    result.position += (initial_.velocity + end_accel_ * constraints_.max_acceleration / 2.0) * end_accel_ +

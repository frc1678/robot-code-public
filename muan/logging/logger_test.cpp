...
class MockFileWriter : public muan::logging::FileWriter {
 public:
  ...
  MOCK_METHOD2(WriteLine,
               void(const std::string& filename, const std::string& line));
  std::ostream& GetTextFile(const std::string& filename) {
...
TEST(Logger, LogsOneMessage) {
  std::unique_ptr<muan::logging::MockFileWriter> writer =
      std::make_unique<muan::logging::MockFileWriter>();

  EXPECT_CALL(*writer, WriteLine("testqueue.csv",
                                 MatchesRegex("thing,queue_index\n42,.*")))
      .Times(1);
...
TEST(Logger, LogsManyMessages) {
  ...
  EXPECT_CALL(*writer, WriteLine("testqueue.csv",
                                 MatchesRegex("thing,queue_index\n42,.*")))
      .Times(1);
  EXPECT_CALL(*writer, WriteLine("testqueue.csv", MatchesRegex("42,.*")))
      .Times(41);
...
TEST(Logger, LogsMultipleQueues) {
  ...
  EXPECT_CALL(*writer, WriteLine("testqueue1.csv",
                                 MatchesRegex("thing,queue_index\n42,.*")))
      .Times(1);
  EXPECT_CALL(*writer, WriteLine("testqueue2.csv",
                                 MatchesRegex("thing,queue_index\n42,.*")))
      .Times(1);
...
TEST(Logger, DiesOnDuplicateQueues) {
  ...
  ASSERT_DEATH(logger.AddQueue("testqueue", &mq2),
               "with same name \"testqueue\"");
}

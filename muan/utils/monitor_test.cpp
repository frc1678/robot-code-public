...
TEST(MotorSafetyTest, InfiniteReset) {
  muan::utils::Monitor safety =
      muan::utils::Monitor(100., 2., std::numeric_limits<double>::max(), 0.01);
  ...
  for (int i = 0; i < 1600; i++) {
    ...
    double current =
        t < 4 ? 200
              : t < 4.5 ? 20
                        : t < 6 ? 200
                                : 20;  // Makes the current fluctuate back above
...
TEST(MonitorTest, BelowThreshold) {
  muan::utils::Monitor monitor =
      muan::utils::Monitor(100., 1., 1., 0.005, false);
...
TEST(MonitorTest, CustomStandingVoltage) {
  muan::utils::Monitor monitor =
      muan::utils::Monitor(100., 1., 1., 0.005, false, 5);
  for (int i = 0; i < 1800; i++) {
    double t = i * 0.005;
    double value = t < 2 ? 200 : 20;
    double voltage = 10;
...
TEST(MonitorTest, CustomHistorySize) {
  muan::utils::Monitor monitor =
      muan::utils::Monitor(100., 1., 1., 0.005, false, 0, 30);
  for (int i = 0; i < 1800; i++) {
    double t = i * 0.005;
    double value = t < 2 ? 200 : 20;
    double voltage = 10;
...
TEST(MonitorTest, Resets) {
  muan::utils::Monitor monitor = muan::utils::Monitor(
      100., 1., std::numeric_limits<double>::max(), 0.005, false, 5);
  for (int i = 0; i < 1800; i++) {
    double t = i * 0.005;
    ...
    if (t < 3.050) {
      EXPECT_NEAR(safe_voltage, voltage, 1e-5);
      EXPECT_FALSE(monitor.is_at_thresh());
    } else if (t < 6) {  // A little more to allow the moving average to catch
                         // up on value
      EXPECT_NEAR(safe_voltage, 5, 1e-5);
      EXPECT_TRUE(monitor.is_at_thresh());
    } else if (t < 6.007) {

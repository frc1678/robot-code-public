...
int ConversionCode(VisionThresholds::ColorSpace from,
  ...
  constexpr int conversion_matrix[3][3] = {{139, CV_RGB2HSV, CV_RGB2BGR},
                                           {CV_HSV2RGB, 139, CV_HSV2BGR},
                                           {CV_BGR2RGB, CV_BGR2HSV, 139}};
...
std::vector<ContourProperties> Vision::Update(cv::Mat raw,
                                              cv::Mat image_canvas) const {
  cv::Mat image;

  cv::cvtColor(raw, image,
               ConversionCode(VisionThresholds::Bgr, range_.space()));
  cv::inRange(image, cv::Scalar(range_.a_low(), range_.b_low(), range_.c_low()),
              cv::Scalar(range_.a_high(), range_.b_high(), range_.c_high()),
              image);
  ...
  for (size_t i = 0; i < contours.size(); i++) {
    ...
    if (base_score > constants_.kMinTargetArea &&
        base_score < constants_.kMaxTargetArea) {
      ...
      target.width = static_cast<double>(bounding.size().width) / image.cols;
      target.height = static_cast<double>(bounding.size().height) / image.rows;

...
void DrivetrainInterface::ReadSensors() {
  o2016::drivetrain::DrivetrainInputProto sensors;
...
IntakeInterface::IntakeInterface()
    : input_queue_(QueueManager::GetInstance().intake_input_queue()),
      ...
      motor_secondaries_{ports::intake::kSecondariesMotor},
...
void IntakeInterface::WriteActuators() {
  ...
  if (outputs) {
    ...
    motor_roller_.Set(
        ...
        12.0);
    motor_secondaries_.Set(
        muan::Cap(-(*outputs)->secondaries_voltage(), -kMaxVoltage, kMaxVoltage) /
        12.0);
  } else {
    motor_pivot_.Set(0.0);
    motor_roller_.Set(0.0);
    motor_secondaries_.Set(0.0);
  }
}

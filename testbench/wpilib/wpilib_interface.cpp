...
void DrivetrainInterface::WriteActuators() {
  ...
  if (outputs) {
    motor_left_.Set(-muan::Cap((*outputs)->left_voltage(), -kMaxVoltage, kMaxVoltage) / 12.0);

    motor_right_.Set(muan::Cap((*outputs)->right_voltage(), -kMaxVoltage, kMaxVoltage) / 12.0);

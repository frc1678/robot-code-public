...
constexpr double kClimberMetersPerClick = muan::units::pi * (1.25 / 2.0) * muan::units::in / 512.0;
...
void SuperstructureInterface::ReadSensors() {
  ...
  auto maybe_current_reader = QueueManager::GetInstance()->pdp_status_queue().ReadLastMessage();

  if (maybe_current_reader) {
    auto current_reader = *maybe_current_reader;
    inputs->set_climber_current(current_reader->current10() + current_reader->current11() +
                                current_reader->current12());
  }
  inputs->set_climber_position(-climber_encoder_.Get() * kClimberMetersPerClick);

  QueueManager::GetInstance()->superstructure_input_queue()->WriteMessage(inputs);
}
...
void SuperstructureInterface::WriteActuators() {
  ...
  if (maybe_outputs) {
    ...
    pcm_->WriteSolenoid(ports::superstructure::kHumanPlayerDeploySolenoid, outputs->hp_gear_extend());
    climber_motor_.Set(muan::utils::Cap(outputs->climber_voltage(), -kMaxVoltage, kMaxVoltage) / 12.0);
  } else {
    pcm_->WriteSolenoid(ports::superstructure::kHumanPlayerDeploySolenoid, false);

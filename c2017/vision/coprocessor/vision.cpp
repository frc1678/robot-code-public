...
double VisionScorer2017::GetScore(double, double /* unused */, double skew, double width, double height,
                                  double fullness) {
...
void RunVision(int camera_index) {
  ...
  muan::Vision::VisionConstants constants{1.14, 0.659, 0.524, 1.66, 1};
  ...
  while (true) {
    ...
    status.image_canvas.copyTo(splitscreen(cv::Rect(0, 0, status.image_canvas.cols,
    status.image_canvas.rows)));
    raw.copyTo(splitscreen(cv::Rect(status.image_canvas.cols, 0, status.image_canvas.cols,
        status.image_canvas.rows)));
#endif

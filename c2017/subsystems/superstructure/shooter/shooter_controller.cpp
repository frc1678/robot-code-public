...
c2017::shooter::ShooterOutputProto ShooterController::Update(c2017::shooter::ShooterInputProto input,
  ...
  shooter_r_ = (Eigen::Matrix<double, 3, 1>() << 0.0,
...
double ShooterController::UpdateProfiledGoalVelocity(double unprofiled_goal_velocity, bool outputs_enabled) {
  if (outputs_enabled) {
    profiled_goal_velocity_ =
        std::min(profiled_goal_velocity_ + kShooterAcceleration, unprofiled_goal_velocity);
  } else {
    profiled_goal_velocity_ = std::max(profiled_goal_velocity_ - kShooterAcceleration, 0.0);
  }

...
class HPolytopeTest : public ::testing::Test {
 protected:
  ...
  HPolytope<2> Polytope4() {
    return HPolytope<2>{
        (Eigen::Matrix<double, 4, 2>() << 1, 1, -1, -1, 1, -1, -1,
         1).finished(),
  ...
  void CheckShiftedVertices(
    ...
    // LOG_MATRIX(DEBUG, "shifted - base", shifted.Vertices() -
    //  base.Vertices());
    EXPECT_THAT(MatrixToVectors(HPolytope<number_of_dimensions>(
                                    base, H_multiplier, k_shifter)
                                    .Vertices()),
...
TEST_F(HPolytopeTest, CalculatedVertices) {
  ...
  EXPECT_THAT(
      ...
      ::testing::UnorderedElementsAreArray(MatrixToVectors(
          (Eigen::Matrix<double, 2, 4>() << 1, 1.5, 1.5, 2, 0, -0.5, 0.5,
           0).finished())));
  EXPECT_THAT(
